...
def get_movie(id):
    ...
    try:
        ...
        pipeline = [
            ...
            {
                # sort the comments in the $lookup stage, jointure entre plusieurs collections
                "$lookup": {"from": "comments", 
                            "let": { "val": "$_id"},# val est crée
                            "pipeline": [ 
                                            { # https://www.mongodb.com/docs/manual/reference/operator/aggregation/literal/
                                                "$match": {# https://www.mongodb.com/docs/manual/reference/operator/aggregation/match/
                                                    "$expr": {
                                                        "$eq": ["$movie_id", "$$val"] # $ is referred to the root document fields where as $$ referred to the variable names.
                                                             } # performs an equality check between the value in the field named movie_id and the value in the field named id in the document.
                                                          }
                                            },
                                            {
                            ...
                            "as": "comments"# alias, Embed the joined comments in a new field called "comments"
                            }
            }
                    ]
